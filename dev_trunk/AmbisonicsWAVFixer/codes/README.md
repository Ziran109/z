...
一个零依赖的 Python GUI 工具，用于批量修复 4 通道 WAV 文件的格式，使其能被 Soundly/Soundminer 正确识别 WYXZ 通道信息。

## 问题背景

当从 DAW（如 Pro Tools、Reaper）导出 4 通道 Ambisonics WAV 文件时，文件通常缺少 iXML 和 bext 元数据，导致 Soundly/Soundminer 无法正确识别通道顺序（WYXZ/WXYZ）。
...
- **通道顺序选择**：支持 ACN (AmbiX: W Y Z X) 和 FuMa (W X Y Z) 两种通道顺序
...
或直接运行 `AmbisonicsWAVFixer_v2.4.exe`

操作流程：
1. 选择包含 WAV 文件的输入文件夹
2. 选择通道顺序：
   - **ACN / AmbiX (W Y Z X)**：默认选项，适用于大多数现代 Ambisonics 工作
   - **FuMa (W X Y Z)**：传统 Furse-Malham 格式
3. 选择输出模式：
   - **原地修改**：直接修改原文件
...
- iXML chunk: 包含通道名称（W Y Z X 或 W X Y Z）
- bext chunk: 包含广播元数据
...
```bash
pyinstaller AmbisonicsWAVFixer_v2.4.spec --clean
```

打包后的文件位于 `dist/AmbisonicsWAVFixer_v2.4.exe`

## 注意事项

1. **备份重要文件**：建议在处理前备份原始文件，或使用"输出到新文件夹"模式
2. **4 通道文件**：工具只处理 4 通道 WAV 文件，其他文件会被跳过
3. **Soundly/Soundminer 识别**：处理后的文件可以在 Soundly/Soundminer 中正确显示 WYXZ/WXYZ 通道信息
...
- 确保 Soundly/Soundminer 能识别 WYXZ 通道信息
